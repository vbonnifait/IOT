...
const char *TOPIC_RAW_EEG = "epilepsy/raw_eeg";       // Signal EEG brut ✓ CORRIGÉ
...
#define SAMPLING_RATE 100         // Hz (BITalino EEG)
...
#define RAW_SIGNAL_INTERVAL_MS 10    // Publier signal brut à 10 Hz
...
unsigned long last_raw_signal_publish = 0;  // ✓ AJOUTÉ pour throttling signal brut
...
void publishRawEEG(int raw_value, float microvolts)
{
    // ✓ PUBLICATION DU SIGNAL BRUT pour Node-RED
        ...
        else if (bt_index > 0 && bt_index < 6)
        {
            ...
            if (bt_index == 6)
            {
                ...
                if (parseBITalinoFrame(bt_buffer, &frame))
                {
                    ...
                    // ✓✓✓ PUBLICATION DU SIGNAL BRUT (throttled à 10 Hz) ✓✓✓
