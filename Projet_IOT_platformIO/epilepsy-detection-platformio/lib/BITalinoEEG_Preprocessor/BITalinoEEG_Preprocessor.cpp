 ...
 * 
 * CORRIGÉ - Compatible avec le nouveau header
 */
...
float BITalinoEEGPreprocessor::convertADCtoMicrovolts(int adc_value) {
    // ADC BITalino: 10 bits (0-1023), VCC = 3.3V
    float voltage = ((float)adc_value / BITALINO_ADC_RESOLUTION) * BITALINO_VCC;
    
    // Conversion en tension EEG (gain = 1000)
...
bool BITalinoEEGPreprocessor::addSample(int adc_value) {
    ...
    // Vérifier si une fenêtre complète est prête
...
bool BITalinoEEGPreprocessor::extractFeatures() {
    ...
    // Features temporelles globales (26 features sur toute la fenêtre)
    extractTemporalFeatures(filtered_buffer, WINDOW_SIZE, feature_idx);
    feature_idx += 26;
    
    // Features par segment (7 segments × 26 features = 182 features)
...
void BITalinoEEGPreprocessor::extractTemporalFeatures(float* segment, int length, int feature_offset) {
    ...
    // 10 features statistiques supplémentaires
    features[feature_offset + 16] = std_val / (mean_val + 1e-8);  // Coefficient de variation
